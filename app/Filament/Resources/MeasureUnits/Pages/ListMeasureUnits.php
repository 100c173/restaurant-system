...
class ListMeasureUnits extends ListRecords
{
    ...
    {
        return MeasureUnitResource::table($table)
            ->recordActions([
                ActionGroup::make([
                    ViewAction::make(),
                    EditAction::make(),
    ...
    protected function getHeaderActions(): array
    {
        return [
            Action::make('import')
                ...
                ->schema([
                    FileUpload::make('file')
                        ->label('Measure units spreadsheet (.xlsx)')
                        ->required()
                        ->disk('local')
                        ->directory('imports/measure-units')
                        ->acceptedFileTypes([
                ...
                ->action(function (array $data, FoodImportExportService $service): void {
                    ...
                    $result = $service->importMeasureUnits($path);

                    Notification::make()
                        ->title('Measure units imported')
            ...
            Action::make('export')
                ...
                ->color('gray')
                ->action(fn (FoodImportExportService $service) => $service->exportMeasureUnits()),
